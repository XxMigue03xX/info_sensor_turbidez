// ESP32 NTU Client — Recolección real con alineación a ticks de 5 s (60 muestras)
// Compatible con API: GET /admin/command  (X-Auth-Token)  y  POST /session
// Payload POST:
// { "session_id": <id>, "readings": [ { "seq": i, "device_epoch_ms": t_i, "ntu": x, "raw_mv": y }, ... ] }
//
// Este sketch replica el comportamiento del simulador Python en modo REAL:
//  - Polling a /admin/command (cada pollSec)
//  - Si command == "start":
//      * Alinea t0 al próximo múltiplo de 5 s del reloj de época (UTC)
//      * Toma 60 mediciones (step 5 s) exactamente en cada tick t_i
//      * Publica el batch a /session al final
//      * Espera postSleepSec y vuelve a hacer polling
//
// Requisitos:
//  - Placa: ESP32 (Arduino core 2.x+)
//  - Librerías: ArduinoJson (>= 6.21), WiFi, HTTPClient, time (incluidas en core),
//               (opcional) esp_adc_cal para analogReadMilliVolts
//  - Sensor turbidez (ej. DFRobot SEN0189) conectado a un pin ADC (ADC1 recomendado)
//  - Si la salida del sensor excede 3.3V, usar divisor resistivo para no dañar el ESP32
//
// NOTA sobre NTU: Se incluye conversión por polinomio típico de SEN0189. Calibre para su sensor.
//  NTU = -1120.4*V^2 + 5742.3*V - 4352.9 (V en voltios a la salida del sensor)
//  Si usa divisor o referencia diferente, actualice la función turbidityFromMilliVolts().
...
String BASE_URL = "http://192.168.1.50/info_sensor_turbidez/api";
String AUTH_TOKEN = "316aee290a9863e1f0de1a6f65eafbe98166771985853ea0"; // X-Auth-Token
...
double turbidityFromMilliVolts(uint16_t mv) {
  ...
  const double DIVIDER_GAIN = 1.0; // ponga 2.0 si usó divisor 1:1, etc.
...
bool postSessionBatch(int sessionId, JsonArray readings) {
  ensureWiFi();
  // Estimar capacidad (readings ya contiene objetos): sólo empaquetamos session_id + readings
  DynamicJsonDocument doc(1024);
  doc["session_id"] = sessionId;
  doc["readings"] = readings; // copia profunda al serializar
  String payload;
  serializeJson(doc, payload);

  String url = buildSessionURL();
  Serial.print("POST "); Serial.print(url); Serial.print(" bytes="); Serial.println(payload.length());
...
void loop() {
  ...
  Serial.print("/admin/command -> "); Serial.println(command);
